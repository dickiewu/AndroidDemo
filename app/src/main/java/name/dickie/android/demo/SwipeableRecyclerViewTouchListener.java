...
/**
 * 滑动删除RecyclerView TouchListener
 */
...
public class SwipeableRecyclerViewTouchListener implements RecyclerView.OnItemTouchListener {
    ...
    private int mAnimationViewId;
    private int mBgViewId;
    private View mBgView;
    private int mChildViewWidth = 0;
     ...
     */
    public SwipeableRecyclerViewTouchListener(RecyclerView recyclerView, int animationViewId, SwipeListener listener) {
        ViewConfiguration vc = ViewConfiguration.get(recyclerView.getContext());
        mAnimationViewId = animationViewId;
    ...
    public SwipeableRecyclerViewTouchListener(RecyclerView recyclerView, int animationViewId,
                                              int bgId, SwipeListener listener) {
        ViewConfiguration vc = ViewConfiguration.get(recyclerView.getContext());
        mAnimationViewId = animationViewId;
        mBgViewId = bgId;
        mSlop = vc.getScaledTouchSlop();
        ...
        switch (motionEvent.getActionMasked()) {
            case MotionEvent.ACTION_DOWN: {
                ...
                for (int i = 0; i < childCount; i++) {
                    ...
                    if (rect.contains(x, y)) {
                        mDownView = child;
                        mAnimationView = child.findViewById(mAnimationViewId);
                        mBgView = child.findViewById(mBgViewId);
                        break;
                    }
                }

                if (mDownView != null && mAnimatingPosition != mRecyclerView.getChildLayoutPosition(mDownView)) {
                    mAlpha = ViewCompat.getAlpha(mAnimationView);
            ...
            case MotionEvent.ACTION_CANCEL: {
                ...
                if (mDownView != null && mSwiping) {
                    // cancel
                    ViewCompat.animate(mAnimationView)
                              .translationX(0)
//                            .alpha(mAlpha)
                              ...
                              .setUpdateListener(new ViewPropertyAnimatorUpdateListener() {
                                ...
                                public void onAnimationUpdate(View view) {
                                    updateChildBgViewSize((int) Math.abs(view.getTranslationX()));
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (dismiss && mDownPosition != mAnimatingPosition && mDownPosition != ListView.INVALID_POSITION) {
                    // dismiss
                    final View downView = mAnimationView; // mDownView gets null'd before animation ends
...
//                            .alpha(mAlpha)
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (deltaX - mSwipingSlop < 0 && !mSwipingLeft) {
                    mSwiping = false;
                }
                if (deltaX - mSwipingSlop > 0 && !mSwipingRight) {
                    mSwiping = false;
                }
                if (mSwiping) {
                    ViewCompat.setTranslationX(mAnimationView, deltaX - mSwipingSlop);
                    updateChildBgViewSize((int) Math.abs(deltaX - mSwipingSlop));
//                    float delta = Math.abs(deltaX) - mViewWidth * 1 / 3;
//                    if (delta < 0) {
//                        delta = 0;
//                    }
//                    delta = delta * 3 / 2;
//                    ViewCompat.setAlpha(mAnimationView, Math.max(0f, Math.min(mAlpha,
//                            mAlpha * (1f - delta / mViewWidth))));
                    return true;
                }
                break;
            }
    ...
    private void updateChildBgViewSize(int width) {
        if (mBgView == null) {
            return;
        }
        ...
        animator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if (mDismissAnimationRefCount == 0) {
                    ...
                    // Reset mDownPosition to avoid MotionEvent.ACTION_UP trying to play a dismiss
